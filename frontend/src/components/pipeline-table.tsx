...
const STAGES: {
  ...
  { key: "diarize", label: "Diarize", icon: UsersIcon, description: "Assign speaker labels with pyannote" },
...
export function PipelineTable({ pipelineState, settings }: PipelineTableProps) {
  ...
  function getConfig(stage: PipelineStage): string {
    switch (stage) {
      ...
      case "diarize":
        return settings.diarization.length > 0 ? settings.diarization.join(", ") : "off";
      case "translate":
